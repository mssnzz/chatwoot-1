...
export default {
  ...
  computed: {
    ...
    agentName() {
      ...
      console.log(this.message.inbox_id);
      /*
      fetch('https://reverent-morse.146-190-54-185.plesk.page/api/bots/' + this.message.inbox_id)
      .then(response => response.json())
      .then(data => {
        this.dataApi = data[0];
        if (this.dataApi.name) {
        return this.dataApi.name;
        } else {
          return 'Bot'
        }
      });
